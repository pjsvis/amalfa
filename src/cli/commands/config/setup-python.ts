...
export const setupPythonCommand = defineCommand({
  meta: {
    name: "setup-python",
    description: "Initialize Python sidecar environment",
  },
  async run() {
    ...
    if (uvCheck.exitCode !== 0) {
      ...
      console.log("  curl -LsSf https://astral.sh/uv/install.sh | ph");
...
export async function cmdSetupPython() {
  if (setupPythonCommand.run) {
    return setupPythonCommand.run({
      rawArgs: [],
      args: { _: [] } as any,
      cmd: setupPythonCommand,
      data: {},
    });
  }
}
