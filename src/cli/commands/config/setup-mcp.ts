...
export const setupMcpCommand = defineCommand({
  meta: {
    name: "setup-mcp",
    description: "Generate MCP configuration JSON",
  },
  async run() {
    ...
    console.log("=".repeat(60));
    console.log(
      "💡 Tip: If you move this folder, run 'amalfa config setup-mcp' again",
    );
    console.log("=".repeat(60));
    console.log();
  },
...
export async function cmdSetupMcp(args: string[]) {
  if (setupMcpCommand.run) {
    return setupMcpCommand.run({
      rawArgs: args,
      args: { _: args } as any,
      cmd: setupMcpCommand,
      data: {},
