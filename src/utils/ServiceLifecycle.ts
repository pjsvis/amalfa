...
export class ServiceLifecycle {
  ...
  async start() {
    ...
    // Write PID file reliably using Node's fs (Bun.write can have sync issues)
    writeFileSync(this.config.pidFile, subprocess.pid.toString());
    subprocess.unref();
  ...
  async serve(serverLogic: () => Promise<void>) {
    ...
    process.on("exit", () => {
      ...
      if (!cleanupCalled && existsSync(this.config.pidFile)) {
        cleanupCalled = true;
        try {
          // Truncate to mark as stale using Node's fs for reliability
          writeFileSync(this.config.pidFile, "");
        } catch {}
      }
    });
