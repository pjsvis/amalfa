...
export const searchCommand = defineCommand({
  meta: {
    name: "search",
    description: "Search knowledge graph using vector and text matching",
  },
  args: {
    query: {
      type: "positional",
    ...
    rerank: {
      type: "boolean",
      description: "Use reranker for higher precision results",
      alias: "r",
    },
  },
  async run({ args }) {
    const { query, json: jsonOutput, rerank: useReranker } = args;
    ...
    try {
      ...
      if (jsonOutput) {
        ...
        if (results.length === 0) {
          ...
          console.log(
            `💡 Tip: Use 'amalfa query read <id>' to view full content of a result\n`,
          );
        }
      }
    } catch (error) {
...
export async function cmdSearch(args: string[]) {
  if (searchCommand.run) {
    // Basic legacy argument parsing
    const query = args.find((arg) => !arg.startsWith("--"));
    let limit = "20";
    const limitIdx = args.indexOf("--limit");
    if (limitIdx !== -1 && args[limitIdx + 1])
      limit = args[limitIdx + 1] ?? "20";

    return searchCommand.run({
      rawArgs: args,
      args: {
        _: args,
        query: query || "",
