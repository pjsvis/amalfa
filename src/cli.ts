...
async function cmdPhi3() {
  ...
  if (!validActions.includes(action)) {
    ...
    console.error("\nUsage: amalfa phi3 <start|stop|status|restart>");
    process.exit(1);
  }

  // Run phi3 daemon with the specified action
  const phi3Path = join(import.meta.dir, "daemon/phi3-agent.ts");
  const proc = spawn("bun", ["run", phi3Path, action], {
    stdio: "inherit",
    cwd: process.cwd(),
  });
