...
const phi3Client: Phi3Client = createPhi3Client();
      ...
      if (name === TOOLS.SEARCH) {
        ...
        try {
          ...
          if (phi3Available && queryAnalysis) {
            ...
            rankedResults = await phi3Client.rerankResults(
              rankedResults,
              query,
              queryIntent,
            );
          ...
          if (phi3Available) {
            ...
            rankedResults = contextResults;
            log.info("✅ Context extraction complete");
          }

          if (rankedResults.length === 0 && errors.length > 0) {
            return {
              content: [
