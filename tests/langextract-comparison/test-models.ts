...
const TEST_FILES: TestFile[] = [
	{
		...
		type: "typescript",
		description: "Ember extraction tool",
	},
	{
		path: "README.md",
		type: "markdown",
	...
	{
		path: "src/core/GraphGardener.ts",
		type: "typescript",
		description: "Graph Gardener logic",
	},
	{
		path: "src/core/VectorEngine.ts",
		type: "typescript",
		description: "Vector Engine implementation",
	},
	{
		path: "src/services/LangExtractClient.ts",
		type: "typescript",
		description: "LangExtract Client service",
	},
	{
		path: "src/core/SidecarSquasher.ts",
		type: "typescript",
		description: "Sidecar adjustment logic",
	},
];
...
async function testModel(
	...
	try {
		...
		if (provider === "gemini") {
			...
			try {
				response =
					await $`curl -s "https://openrouter.ai/api/v1/chat/completions" -X POST -H "Content-Type: application/json" -H "Authorization: Bearer ${apiKey}" -H "HTTP-Referer: https://github.com/pjsvis/amalfa" -H "X-Title: AMALFA Model Test" -d @${tempFile}`;
			} finally {
				unlinkSync(tempFile);
			}
		} else {
