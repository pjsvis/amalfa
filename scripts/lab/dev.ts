...
async function runServer() {
  ...
  const server = Bun.serve({
    ...
    async fetch(req) {
      ...
      if (path === "/api/health") {
        try {
          const { DatabaseFactory } =
            await import("../../src/resonance/DatabaseFactory");
      ...
      // API: Markdown Rendering (Bun v1.3.8+)
      if (path === "/api/render") {
        try {
          ...
          if (!markdown) {
            return new Response(
              JSON.stringify({ error: "markdown field required" }),
              { status: 400 },
            );
          }

          const html = (Bun as any).markdown.html(markdown);

          return new Response(
            JSON.stringify({ html, cached: false, renderTime: 0 }),
            { headers: { "Content-Type": "application/json" } },
          );
        } catch (e) {
          console.error("API Render Error:", e);
          return new Response(
            JSON.stringify({ error: "Failed to render markdown" }),
            { status: 500 },
          );
        }
      }
