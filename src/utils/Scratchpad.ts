...
export interface ScratchpadConfig {
	/** Minimum size (bytes) to trigger caching. Default: 4KB */
	thresholdBytes: number;
	/** Maximum age (ms) before entries are pruned. Default: 24 hours */
	maxAgeMs: number;
	/** Maximum total cache size (bytes). Default: 50MB */
	maxCacheSizeBytes: number;
	/** Whether to include preview in reference. Default: true */
	includePreview: boolean;
	/** Preview length (chars). Default: 200 */
	previewLength: number;
}

const DEFAULT_CONFIG: ScratchpadConfig = {
	thresholdBytes: 4 * 1024, // 4KB
 ...
 */
export function getScratchpad(config?: Partial<ScratchpadConfig>): Scratchpad {
	if (!_instance) {
